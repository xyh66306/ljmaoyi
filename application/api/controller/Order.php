...
class Order extends Api
{
    ...
    public function aftersalesInfo()
    {
        ...
        $pointFilter                     = [];
        $pointFilter[]                   = ['user_id', '=', $this->userId];
        $pointFilter[]                   = ['type', '=', $userPointLogModel::POINT_TYPE_AFTERSALES];
        $pointFilter[]                   = ['remarks', 'like', '%' . $info['data']['order_id'] . '%'];
        $info['data']['points_refunded'] = $userPointLogModel->where($pointFilter)->sum('num');
        
          //获取订单状态
        $orderModel                      = new orderModel();
        $owhere[]                        = ['order_id', 'eq', $info['data']['order_id']];
        $orderStatus                     = $orderModel->field('status,point')->where($owhere)->find();
        $info['data']['order_status']    = $orderStatus['status'];
        $info['data']['points_refunded'] = $info['data']['points_refunded'] ? $info['data']['points_refunded'] : $orderStatus['point'];
