    ...
    public function getList()
    {
        ...
        if (input('?param.where','','safe_filter')) {
			...
			if (isset($postWhere['search_name']) && $postWhere['search_name']) {
                if(stripos($postWhere['search_name'],' ')!==false){
                    $search_key = explode(" ",$postWhere['search_name']);
                    foreach($search_key as $sk){
                         $where[] = ['g.name|g.bn|g.brief', 'LIKE', '%' . $sk . '%'];
                    }
                }else{
            ...
            // 积分商品类型筛选
            if (isset($postWhere['open_point']) && $postWhere['open_point']) {
                if(is_array($postWhere['open_point'])){
                    $where[] = ['g.open_point', 'in', $postWhere['open_point']];
                }else{
                    $where[] = ['g.open_point', '=', $postWhere['open_point']];
                }
              
                if (isset($postWhere['exchange']) && $postWhere['exchange'] == 1) { //我能兑换
                    $user_id = getUserIdByToken(input('token'));
                    if ($user_id) {
                        $userPoint = get_user_info($user_id, 'point');
                        $where[] = ['g.point', '<=', $userPoint];
                    }
                }
            }else{
