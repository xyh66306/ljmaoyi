...
class User extends Api
{
    ...
    public function smsLogin()
    {
        ...
        //验证绑定来源是否正常
        if (!input('param.uuid') && input('param.user_wx_id')) {
            return error_code(100013);
        }
        if (input('param.uuid') && input('param.user_wx_id')) {
            $uuid             = input('param.uuid');
            $user_wx_id       = input('param.user_wx_id');
            $uuid_auth        = md5($uuid . '_' . config('jshop.product') . '_' . $user_wx_id);
            $server_uuid_auth = Cache::tag('user')->get('uuid_auth_' . $uuid . '_' . $user_wx_id);
            if ($uuid_auth != $server_uuid_auth) {
                return error_code(100014);
            }
        }
    ...
    public function wxappLogin2()
    {
        ...
        $result =  $wxapp->updateWxInfo(input('param.open_id','','trim'), input('param.edata','','trim'), input('param.iv','','trim'), $pid);

        $uuid = input('param.uuid', '');  //前端匿名用户id
        if ($uuid && (isset($result['data']['openid']) || $result['data']['user_wx_id'])) {
            $uk        = isset($result['data']['openid']) ? $result['data']['openid'] : $result['data']['user_wx_id'];
            $uuid_auth = md5($uuid . '_' . config('jshop.product').'_'.$uk);
            Cache::tag('user')->set('uuid_auth_' . $uuid . '_' . $uk, $uuid_auth, 600);  //todo 注意缓存key，后台清空时，这个也会同时被清，下次优化后台清除缓存
        }
    ...
    public function sms()
    {
        ...
        if (config('jshop.captcha')) {
            $captcha    = input('captcha');
            $captcha_id = input('captcha_id');
            if (!$captcha || !$captcha_id) {
                return error_code(10105);
            }
            if (!captcha_check($captcha, $captcha_id)) { //验证码错误的时候，不发送短信
                return error_code(10106);
            }
        }
    ...
    public function officialLogin()
    {
        ...
        $wx     = new Wxofficial();
        $result = $wx->codeToInfo(input('param.code'), input('param.state'), $scope, $pid);
    ...
    public function pay()
    {
        ...
        if (!Cache::has($lock_key)) {
            ...
            $billPaymentsModel = new BillPayments();
            //生成支付单,并发起支付
            $payRes = $billPaymentsModel->pay(input('param.ids'), input('param.payment_code'), $user_id, input('param.payment_type'), $params);
            Cache::rm($lock_key);
            return $payRes;
        }else{
            return $return_data;
    ...
    public function pointRanking()
    {
        ...
        $page      = input('page', 1,'safe_filter');
        $limit     = input('limit', 10,'safe_filter');
        $userModel = new \app\common\model\User();
        $start     = ($page - 1) * $limit;

        $sql  = 'select u.id,u.nickname,u.avatar,u.point,u.grade,u.ctime ,@rownum := @rownum + 1 AS rownum from  (SELECT @rownum := 0) r ,' . config('database.prefix') . 'user as u   order by u.point desc limit ' . $start . ',' . $limit . ';';
        $list = $userModel->query($sql);

        $count = $userModel->count();

        if($page == 1){
            $currentUserSql = 'SELECT b.* from ( SELECT u.id,u.nickname,u.avatar,u.point,u.grade,u.ctime, @rownum := @rownum + 1 AS rownum FROM (SELECT @rownum := 0) r, ' . config('database.prefix') . 'user AS u ORDER BY u.point DESC ) as b where b.id = ' . $this->userId . ';';
            $user = $userModel->query($currentUserSql);
        }
        if ($list) {
            $gradeModel = new UserGrade();
            foreach ($list as $key => $value) {
                $list[$key]['rownum']     = ($page-1)*$limit+$value['rownum'];
                $list[$key]['avatar']     = _sImage($value['avatar']);
                $list[$key]['ctime']      = getTime($value['ctime'], true);
                $list[$key]['grade_name'] = $gradeModel->where('id', $value['grade'])->cache('user_grade_' . $value['grade'], 86400)->value('name');
            }
            $current_user                   = $user[0];
            $current_user['avatar']         = _sImage($current_user['avatar']);
            $current_user['ctime']          = getTime($current_user['ctime'],true);
            $current_user['grade_name']     = $gradeModel->where('id', $current_user['grade'])->cache('user_grade_' . $current_user['grade'], 86400)->value('name');
    ...
    public function getAddressData()
    {
        ...
        $pid = input('param.pid', '0'); //父级
        $level = input('param.level', '1'); //级别

        $data = Db::name('area')->where(['parent_id' => $pid, 'depth' => $level])->order('sort asc,id asc')->field('id,name')->select()->toArray();
    ...
    public function getProfit()
    {
        ...
        $UserModel = new UserModel();
        $return_data = $UserModel->getProfitData($uid);
    ...
    public function getMonthEarnings()
    {
        ...
        $user_id = $this->userId;
        $date = input('param.month', '');
        $UserModel = new UserModel();
        $return_data = $UserModel->getEarningsData($user_id, $date, '1');
        if ($return_data['status']) {
            $return_data['status']             = true;
            $return_data['msg']                = '查询成功';
        }
    ...
    public function getDayEarnings()
    {
        ...
        $expire = input('param.expire', '');
        $UserModel = new UserModel();
        $return_data = $UserModel->getEarningsData($user_id, $date,'',$expire);
    ...
    public function getMyCenData()
    {
        ...
        $user_id = $this->userId; //获取user_id

        $UserModel = new UserModel();
        $return_data = $UserModel->getMyCenData($user_id);
        if ($return_data['status']) {
            $return_data['status']             = true;
            $return_data['msg']                = '查询成功';
        }
