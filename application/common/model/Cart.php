...
class Cart extends Common
{
    ...
    const TYPE_BARGAIN  = 9;   //砍价模式
    const TYPE_POINT    = 10;  //积分兑换订单
    const TYPE_PINTUAN  = 11;  //拼团模式
    const TYPE_GIVEAWAY = 12;  //赠品，在cart表里不会存在，但是会在计算促销过之后，动态的加上去
...
//                            $result['msg'] = '您已超过该活动最大购买量';
            ...
            case self::TYPE_POINT:
                $num_type = 2;
                //此人的购物车中的所有购物车关于积分兑换的商品删除
                $delWhere[] = ['user_id', 'eq', $user_id];
                $delWhere[] = ['type', 'eq', self::TYPE_POINT];
                $this->where($delWhere)->delete();
                unset($cat_info);
                break;
            default:
...
//                $result['msg'] = '库存不足';
    ...
    public function info($userId, $ids, $order_type = '1',$display = false, $area_id = 0, $point = 0, $coupon_code = "", $free_freight = false, $delivery_type = 1,$params = [])
    {
        ...
        if ($order_type == self::TYPE_COMMON) {
            ...
            $promotionModel = new Promotion();
            $promotionModel->toPromotion($result['data']);
        } elseif ($order_type == self::TYPE_POINT) {//积分抵扣订单
            $point                   = $result['data']['list'][0]['products']['point']*$result['data']['list'][0]['nums'];
            $result['data']['point'] = $point;
            //获取可用积分
            $userModel = new User();
            $where[] = ['id', 'eq', $userId];
            $data    = $userModel->field('point')->where($where)->find();
            $result['data']['available_point'] = $data['point'];//可用积分
        }
        ...
        //使用积分
	if ($order_type != self::TYPE_POINT) {//积分抵扣订单
