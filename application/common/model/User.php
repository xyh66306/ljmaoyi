...
class User extends Common
{
    ...
    public function getProfitData($user_id)
    {
        ...
        $data['commission'] =  $this->where('id', $user_id)->value('team_value');
        $data['today'] = Db::name('user_fenyong')->where(['receipt_id' => $user_id, 'is_can_fenyong' => 0,  'is_del' => 0])->whereTime('ctime', 'today')->sum('money');
        $data['count'] = Db::name("user")->where(['pid'=>$user_id])->count('id');
     ...
     * @return void
     */
    public function getEarningsData($user_id = '', $date = '', $type = '',$expire='')
    {
        $return_data = [
            'status' => false,
            'msg'    => '查询失败',
        ...
        if ($type == '1') {
            ...
            #总收益
            $list = Db::name('user_fenyong')
                ->where(['receipt_id' => $user_id, 'is_del' => 0, 'is_can_fenyong' => 0])
                ->whereBetweenTime('ctime', $start_time, $end_time)
                ->field('ctime,money')
                ->order('ctime desc')
                ->select()
                ->toArray();
            ...
            if (!empty($list)) {
                ...
                foreach ($list as $key => $v) {
                    $new_list[date('Y-m-d', $v['ctime'])][] = $v['money'];
                }
                if (!empty($new_list)) {
                    foreach ($new_list as $key => $v) {
                        $result_list[] = [
                            'date' => $key,
                            'money' => round(array_sum($v), 2),
                            'show'  => false,
                        ];
                    }
                }
            ...
            if (!empty($result_list)) {
                $return_data['data']['sum'] = round(array_sum(array_column($result_list, 'money')), 2);
            } else {
                $return_data['data']['sum'] = 0;
            }
        } else {
            ...
            // $map[] = ['a.type','>=',2];
            // $map[] = ['a.is_del','=',0];
            $map[] = ['a.receipt_id','=',$user_id];
            if(is_numeric($expire)){
                $map[] = ['a.expire','=',$expire];
            }


            $list = Db::name('user_fenyong')->alias('a')
                ->join('user c', 'c.id = a.contribute_id', 'LEFT')
                ->where($map)
                ->whereBetweenTime('a.ctime', $start_time, $end_time)
                ->field('a.id,a.money,a.contribute_grade,a.order_id,a.ctime,a.type,a.expire,c.nickname, c.avatar')
                ->order('a.ctime desc')
                ->select()
                ->toArray();
        ...
        $return_data['status'] = true;
        $return_data['data']['list'] = $result_list;
        $return_data['msg'] = '查询成功';
    ...
    public function getMyCenData($user_id)
    {
        $return_data = [
            'status' => false,
            'msg'    => '查询失败',
        ...
        $balance = $userFenyong->where(['receipt_id' => $user_id, 'is_del' => 0, 'is_can_fenyong' => 0])->sum('money');
