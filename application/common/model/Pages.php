...
class Pages extends Common
{
     ...
     * @return array
     */
    public function getHomeDetails($token = '',$page = 1,$limit = 5)
    {
        $result          = [
            'status' => true,
            'msg'    => '获取成功',
        ...
        $data  = $pagesItemsModel->where([['page_code', '=', $result['data']['code']]])->page($page)->limit($limit)->order('sort asc')->cache(86400)->select();
        $total = $pagesItemsModel->where([['page_code', '=', $result['data']['code']]])->cache(86400)->count();
            ...
            foreach ($data as $key => $value) {
                ...
                if ($value['widget_code'] == 'notice') {
                    ...
                    if ($data[$i]['params']['type'] == 'auto') {
                        ...
                        $where[]                    = ['g.open_point','eq',$goodsModel::OPEN_POINT_NO];//关闭积分
     ...
     * @return array
     */
    public function getDetails($page_code, $token = '', $page = 1, $limit = 5)
    {
        $result          = [
            'status' => true,
            'msg'    => '获取成功',
        ...
        $data            = $pagesItemsModel->where([['page_code', '=', $page_code]])->page($page)->limit($limit)->order('sort asc')->cache(86400)->select();
        $total = $pagesItemsModel->where([['page_code', '=', $page_code]])->cache(86400)->count();
