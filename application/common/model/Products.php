...
class Products extends Common
{
    ...
    public function getProductInfo($id, $isPromotion = true, $user_id = 0, $type = 'goods',$params = [])
    {
        ...
        $goods = $goodsModel->where(['id' => $product['goods_id']])->field('name,image_id,bn,marketable,spes_desc,is_combo,point,open_point')->find(); //后期调整
        //判断如果没有商品，就返回false
        if (!($goods)) {
        ...
        $product['point']      = $goods['point'];
        $product['open_point'] = $goods['open_point'];
        $product['image_id'] = (isset($product['image_id']) && $product['image_id'] != '') ? $product['image_id'] : $goods['image_id'];
        $product['bn']       = $goods['bn'];
        if (!$product['image_id']) {
