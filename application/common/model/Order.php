...
class Order extends Common
{
    ...
    const ORDER_TYPE_RECHARGE = 2;  //订单类型，2充值

    const ORDER_TYPE_GROUP = 3;  //订单类型，3团购
    const ORDER_TYPE_SKILL = 4;  //订单类型，4秒杀

    const ORDER_FORM_ORDER   = 5;   //订单类型，5 表单订单
    const ORDER_FORM_PAY     = 6;   //订单类型,6表单付款码
    const ORDER_TYPE_LOTTERY = 7;   //订单类型，7抽奖订单
    const ORDER_TYPE_COMBO   = 8;   //订单类型，8免单活动订单
    const ORDER_TYPE_BARGAIN = 9;   //订单类型，9砍价订单
    const ORDER_TYPE_POINT   = 10;  //订单类型，积分
    const ORDER_TYPE_PINTUAN = 11;  //订单类型，11拼团订单
    ...
    protected function getListByWhere($input, $isPage = true)
    {
        ...
        if (isset($input['store_id']) && !empty($input['store_id'])) {
            $where[] = ['o.store_id', 'eq', $input['store_id']];
        }
        ...
        if ($isPage) {
            ...
            $data = $this::with('aftersales')->alias('o')
                ->field('o.order_id, o.point, o.user_id, o.ctime, o.ship_mobile, o.ship_address, o.status, o.pay_status, o.ship_status, o.confirm, o.is_comment, o.order_amount, o.source, o.ship_area_id,o.ship_name, o.mark,o.order_type,o.store_id')
    ...
    public function getListFromAdmin($input, $isPage = true)
    {
        ...
        if (count($result['data']) > 0) {
            ...
            foreach ($result['data'] as $k => &$v) {
                ...
                $store = getStoreInfo(['id' => $v['store_id']], 'store_name');
                $v['store_name'] = $store ? $store['store_name'] : '';
...
//        $order_info['express_delivery'] = $express_delivery;
    ...
    {
        $total_order_pmt         = $order_pmt = $orderInfo['order_pmt'] + $orderInfo['point_money'];//上面运算时，订单优惠和积分优惠没有合并，在这里合并一下
        $total_point             = $item_point = $orderInfo['point'];
        $total_point_money       = $orderInfo['point_money'];
        $totalItems              = count($orderInfo['items']);
        $accumulated_pmt         = 0;                                                    //累计优惠
        $accumulated_point       = 0;                                                    //累计积分
        $accumulated_point_money = 0;                                                    //累计积分金额
        
        foreach ($orderInfo['items'] as $k => &$v) {
            if (!$orderInfo['goods_amount'] || $orderInfo['goods_amount'] == 0) {
                $order_pmt = 0;
            } else if ($totalItems == ($k - 1)) {
                $order_pmt        = $total_order_pmt - $accumulated_pmt;            //计算最后一次优惠
                $item_point       = $total_point - $accumulated_point;              //计算最后一次积分数量
                $item_point_money = $total_point_money - $accumulated_point_money;  //计算最后一次积分金额
            } else if ($totalItems != ($k - 1)) {
                $order_pmt          = $total_order_pmt * ($v['amount'] / $orderInfo['goods_amount']);        //计算优惠占比
                $accumulated_pmt   += $order_pmt;
                $item_point         = round($total_point * ($v['amount'] / $orderInfo['goods_amount']), 0);
                $accumulated_point += $item_point;
                $item_point_money         = round($total_point_money * ($v['amount'] / $orderInfo['goods_amount']), 2);
                $accumulated_point_money += $item_point_money;
            }
            $v['ave_price']  = round(($v['amount'] - $order_pmt) / $v['nums'], 2);
            $v['ave_amount'] = round(($v['amount'] - $order_pmt), 2);
            $v['point']      = $item_point;
            $v['item_point'] = round(($item_point / $v['nums']), 0);;
            $v['point_fee']  = $item_point_money;
        }
    }
...
//            $billDeliveryModel->confirm($order_id);
    ...
    public function toAdd($user_id, $order_type, $cart_ids, $delivery, $memo, $point = 0, $coupon_code = false, $formId = false, $source = 2, $tax = [], $params = [])
    {
        ...
        //检查积分订单
        if ($items) {
            if ($order_type != self::ORDER_TYPE_POINT) {
                foreach ($items as $key => $val) {
                    if (in_array($val['open_point'], ['1', '2'])) {
                        $result['msg']    = '普通订单不能下积分兑换商品';
                        $result['data']   = '';
                        $result['status'] = false;
                        return $result;
                    }
        ...
        //判断积分兑换是否开启
        if ($order_type == self::ORDER_TYPE_POINT){
            $open_point_exchange = getSetting('open_point_exchange');
            if($open_point_exchange == 2){
                $result['msg']    = '积分订单暂时无法兑换';
                $result['data']   = '';
                $result['status'] = false;
                return $result;
            }
        ...
        try {
            ...
            switch ($order_type) {
                ...
                case self::ORDER_TYPE_BARGAIN:
                    ...
                    $bo_re = $bargainRecordModel->updateRecord($record_id, ['order_id' => $order['order_id'], 'status' => $bargainRecordModel::STATUS_HAVE_ORDER]);
                    if (!$bo_re) {
                        Db::rollback();
    ...
    private function formatOrder(&$order, $user_id, $cart_ids, $area_id, $point, $coupon_code, $free_freight = false, $delivery_type = '1',$params = [])
    {
        ...
        //积分使用情况
        if($order['order_type'] == self::ORDER_TYPE_POINT){
            $order['point'] = $cartInfo['data']['list'][0]['products']['point']*$cartInfo['data']['list'][0]['nums'];
            $order['point_money'] = 0;
        }else{
        ...
        if (!$items) {
            return error_code(12021);  //判断订单明细为空的不能下单
        }
        $orderInfo          = $order;
        $orderInfo['items'] = $items;
        $this->avePrice($orderInfo);  //创建计算时计算分摊
        return [
            'status' => true,
            'data' => $orderInfo['items'],           //订单主体表通过引用直接返回值，订单明细通过这里返回值
            'msg' => ''
        ];
    }
    ...
    private function formatOrderItems($list, $order_id)
    {
        ...
        foreach ($list as $v) {
            ...
            $item['open_point'] = $v['products']['open_point'];
            $item['item_point'] = $v['products']['point'];
            
            // 是否免单商品
            $item['is_free'] = isset($v['products']['is_free']) ? $v['products']['is_free'] : 0;
