...
class BillPayments extends Common
{
    ...
    const TYPE_GROUP    = 3;  //单据类型 团购订单
    const TYPE_SKILL    = 4;  //单据类型 秒杀订单

    const TYPE_FORM_ORDER = 5;  //单据类型 表单订单
    const TYPE_FORM_PAY   = 6;  //单据类型 表单付款码


    const TYPE_LOTTERY = 7;  //单据类型，7抽奖订单
    const TYPE_COMBO   = 8;  //单据类型，8免单活动订单
    const TYPE_BARGAIN = 9;  //单据类型，9砍价订单

    const TYPE_POINT   = 10;  //单据类型 积分订单
    const TYPE_PINTUAN = 11;  //单据类型 拼团订单
...
//            $result['msg'] = '没有找到此未支付的支付单号';
        ...
        try {
            ...
            if($status == self::STATUS_PAYED){
                ...
                if($billPaymentInfo['type'] == self::TYPE_ORDER || $billPaymentInfo['type'] == self::TYPE_POINT || $billPaymentInfo['type'] == self::TYPE_BARGAIN || $billPaymentInfo['type'] == self::TYPE_COMBO || $billPaymentInfo['type'] == self::TYPE_GROUP || $billPaymentInfo['type'] == self::TYPE_SKILL || $billPaymentInfo['type'] == self::TYPE_PINTUAN){
                    //如果是订单类型，做支付后处理
...
//                'rel' => array()
        ...
        if ($type == self::TYPE_ORDER || $type == self::TYPE_GROUP || $type == self::TYPE_SKILL || $type == self::TYPE_LOTTERY || $type == self::TYPE_COMBO || $type == self::TYPE_BARGAIN || $type == self::TYPE_POINT || $type == self::TYPE_PINTUAN) {
