...
class Goods extends Common
{
    ...
    const OPEN_POINT_ALLOW_CASH_YES = 1;//积分+现金
    const OPEN_POINT_JUST_POINT = 2;//积分
    const OPEN_POINT_NO = 3;//关闭积分抵扣
	...
	if (isset($post['open_point']) && $post['open_point'] != "") {
            $where[] = ['open_point', '=', $post['open_point']];
        }
    ...
    protected function tableFormat($list)
    {
        ...
        foreach ($list as $key => $val) {
            ...
            $products = $productModel->where([['goods_id', '=', $val['id']]])->field('spes_desc,stock,freeze_stock')->select()->toArray();
            if (count($products) > 1) {
                $specStocks = [];
                foreach ($products as $p) {
                    $stock = ($p['stock'] - $p['freeze_stock']) > 0 ? $p['stock'] - $p['freeze_stock'] : 0;
                    $specStocks[] = $p['spes_desc'] . '库存:' . $stock;
                }
                $list[$key]['stock'] = implode(',', $specStocks);
            } else {
                $list[$key]['stock'] = ($products[0]['stock'] - $products[0]['freeze_stock']) > 0 ? ($products[0]['stock'] - $products[0]['freeze_stock']) : 0;
            }
    ...
    public function getGoodsDetial($gid, $fields = '*', $token = '', $type = 'goods', $params = [])
    {
        ...
        if ($list) {
            ...
            //图片处理
            if (isset($list['intro'])) {
                $list['intro'] = stripslashes($list['intro']);
     ...
     * @return array
     */
    public function getPointDetial($gid, $fields = '*', $token = '', $type = 'goods', $params = [])
    {
        $result        = [
            'status' => true,
            'data'   => [],
        ...
        $list = $this::with($preModel)->field($fields)->where('open_point', 'in', [1, 2])->where(['id' => $gid])->find();
        if ($list) {
            if (isset($list['brand_id']) && $list['brand_id']) {
                $list->brand;
            }

            if (isset($list['video_id'])) {
                $videoModel    = new Videos();
                $video         = $videoModel->videoDetial($list['video_id'], 1);
                $list['video'] = $video['data'];
            }
            ...
            $list['price']   = $list['product']['price'];
            if ($list['spes_desc']) {
                $list['spes_desc'] = json_decode($list['spes_desc'], true);
            }
            //取出图片集
            $imagesModel = new GoodsImages();
            $images      = $imagesModel->where(['goods_id' => $list['id']])->order('sort asc')->select();
            ...
            $list           = $this->filterFields($list, 'goods');
            $product_ids = $productsModel->where('goods_id', $gid)->column('id');
            $list['product_ids'] =  implode(",", $product_ids);
            $result['data'] = $list;
        }
        return $result;
    }
