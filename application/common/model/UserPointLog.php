...
class UserPointLog extends Common
{
    ...
    const POINT_TYPE_IMPORT     = 7;  //导入积分
    const POINT_TYPE_CANCEL     = 8;  //过期作废
    const POINT_TYPE_AFTERSALES = 9;  //订单退还
    ...
    public function setPoint($user_id, $num, $type = self::POINT_TYPE_SIGN, $remarks = '')
    {
        if ($num != 0) {
            ...
            $data = [
                ...
                'type'        => $type,
                'num'         => $num,
                'surplus_num' => $num > 0 ? $num : 0,//剩余积分数量
                'balance'     => $new_point,
                'remarks'     => $remarks,
                'ctime'       => time()
            ];
            ...
            $user_info->where($where)
                ->setInc('point', $num);
            if ($num < 0) {
                $this->clearPoint($user_id, $num);//日志也减掉
            }
        }
        //重置会员缓存
        $delKey = 'user_info_' .$user_id;
    ...
    //减去某用户剩余积分
    public function clearPoint($user_id = 0, $point = 0)
    {
        if (abs($point) > 0) {
            $info = $this->where('user_id', '=', $user_id)->order('ctime', 'asc')->where('surplus_num', '>', '0')->find();
            if ($info) {
                $suplusNum = abs($point) - $info['surplus_num'];
                if ($suplusNum > 0) {
                    $this->where('id', '=', $info['id'])->update(['surplus_num' => 0]);
                    $this->clearPoint($user_id, $suplusNum);
                } elseif ($suplusNum < 0) {
                    $this->where('id', '=', $info['id'])->update(['surplus_num' => abs($suplusNum)]);
                }
            }
        }
    }
    ...
    protected function signFixedPointCalculation($user_id)
    {
        ...
        $res     = $this->fieldRaw('DATE_FORMAT(FROM_UNIXTIME(ctime), "%Y-%m-%d") as day')
            ->where($where)
    ...
    public function pointLogList($user_id, $type = false, $page = 1, $limit = 20,$params = [])
    {
        ...
        if($params){
            ...
            if(isset($params['date_string']) && !empty($params['date_string'])){
                $sdate='';
                $edate= time();
                if($params['date_string'] == 'one'){
                    $sdate = time()-86400*30;
                }elseif($params['date_string'] == 'three'){
                    $sdate = time()-86400*30*3;
                }elseif($params['date_string'] == 'year'){
                    $sdate = strtotime(date('Y').'-01-01' . ' 00:00:00');
                }
                $where[] = ['ctime', ['>=', $sdate], ['<=', $edate], 'and'];
            }

            if(isset($params['point_type_string']) && !empty($params['point_type_string'])){
                if($params['point_type_string']=='1'){
                    $where[] = ['num','>','0'];
                }elseif($params['point_type_string']=='-1'){
                    $where[] = ['num','<','0'];
                }
            }
        ...
        if ($res) {
            ...
            if (count($res) >= 1) {
                ...
                foreach ($return['data'] as &$v) {
                    ...
                    $v['remarks'] = $v['remarks']?$v['remarks']:$v['type'];
                }
            } else {
                $return['msg'] = '暂无积分记录';
            }
