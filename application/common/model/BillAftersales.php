        ...
        try {
            ...
            if ($status == self::STATUS_SUCCESS) {
                ...
                if ($refund > 0) {
                    ...
                    $paymentType = isset($orderInfo['paymentRelItem'][0]['bill']['type']) ? $orderInfo['paymentRelItem'][0]['bill']['type'] : $orderInfo::TYPE_ORDER;
                    $refund_re       = $billRefundModel->toAdd($info['user_id'], $info['order_id'], $paymentType, $refund, $info['aftersales_id']);
                    if (!$refund_re['status']) {
                        Db::rollback();
                        return $refund_re;
                    }
                ...
                //如果货全部退完了，并且订单使用积分了，那么去退积分
                if($all_sened && $orderInfo['point'] > 0){
                    $pointLogMode = new UserPointLog();
                    $pointLogMode->setPoint($orderInfo['user_id'], $orderInfo['point'], $pointLogMode::POINT_TYPE_AFTERSALES, '取消订单：' . $orderInfo['order_id'] . ' 返还积分');    
                }
            }
            Db::commit();
