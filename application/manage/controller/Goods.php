...
class Goods extends Manage
{
    ...
    public function doAdd()
    {
        ...
        if ($open_spec) {
            ...
            foreach ($product as $key => $val) {
                $tmp_product['goods']['price']        = $data['goods']['open_point'] == 2 ? 0 : (isset($val['price']) ? $val['price'] : 0);
    ...
    private function checkGoodsInfo($isEdit = false)
    {
        ...
        $data['goods']['open_point']    = input('post.goods.open_point', '');
        $data['goods']['point']         = $data['goods']['open_point'] == 3 ? 0 : input('post.goods.point', '');
        ...
        $data['goods']['intro']        = input('post.goods.intro', '','addslashes');
        $data['goods']['price']        = $data['goods']['open_point'] == 2 ? 0 : input('post.goods.price', '');
...
//            $result['msg'] = error_code(12009,true);
        ...
        $data['product']['price']      = $data['goods']['open_point'] == 2 ? 0 : (isset($data['goods']['price']) ? $data['goods']['price'] : 0);//货品价格
    ...
    public function doBatchModifyPrice()
    {
        ...
        $ids = $goodsModel->where([['id', 'in', $ids]])->where('open_point', 'in', ['1', '3'])->column('id');
        if (!$ids) {
            return [
                'status' => true,
                'data'   => '',
