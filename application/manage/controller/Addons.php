...
class Addons extends Manage
{
    ...
    public function refresh()
    {
        ...
        if ($data) {
            ...
            $apiMenu   = isset($config['api']) ? $config['api'] : [];
            $config         = array_merge($config, $oldConfig);
            if($menu) $config['menu'] = $menu; //菜单使用最新的
            if($apiMenu) $config['api']  = $apiMenu; //菜单使用最新的
