		...
		methods: {
			...
			gotoStore(id) {
				if (id && id != '') {
					...
					let _this = this;
					setTimeout(() => {
						if((this.$store.state.config && this.$store.state.config.store_switch) == 2) {
							url = '/pages/index/index'
						}
						uni.reLaunch({
							url:url
			...
			gotoBargain(id, type, record_id) {
				if (id && id != '' && type && record_id && record_id != 0) {
					let url = '/pageactivity/bargain/index?id=' + id + '&type=' + type + '&record_id=' + record_id;
					// console.log(url);
					let _this = this;
					setTimeout(() => {
						uni.reLaunch({
