<template>
	<view class="content">
		...
		<view v-if="!closeMobileLogin && weixinBrowser">
			<view class="login-m">
		...
		<!--  微信浏览器里 -->
		<!-- #ifdef H5 -->
		<template v-if="weixinBrowser && !closeMobileLogin">
		...
		<template v-if="weixinBrowser && closeMobileLogin">
			...
			<view class="flc third-block">
				...
				<view class="fz12 g5" style="text-align: center;margin-top: 20px;color: #000;font-size: 14px;">微信登录</view>
		</template>
		<!-- #endif -->
	...
	export default {
		...
		data() {
			return {
				...
				closeMobileLogin:closeMobileLogin
			};
		},
		onLoad(options) {
