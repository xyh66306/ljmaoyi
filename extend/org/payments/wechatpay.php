...
class wechatpay implements Payment
{
    ...
    public function pay($paymentInfo){
        ...
        $data['total_fee'] = ceil(((int)($paymentInfo['money']*1000))/10);                        //总金额
        $data['fee_type'] = 'CNY';
    ...
    public function refund($refundInfo,$paymentInfo){
        ...
        $data['total_fee'] = ceil(((int)($paymentInfo['money']*1000))/10);                        //订单总金额
        $data['refund_fee'] = ceil(((int)($refundInfo['money']*1000))/10);         //退款金额
...
//        }
     ...
     * todo 已作废
     */
    public function pay_bank($transfersInfo){
        $result = [
            'status' => false,
            'data' => [],
            'msg' => ''
        ...
        if(
            ...
            $result['status'] = false;                       //没有证书的时候，相当于客户没有配置证书，那么原路返回线上就直接做已退款操作就可以了，不实际去退了，具体的金额在支付后台做退款
            $result['msg'] = "微信支付证书没有上传，退回到用户余额";
            return $result;
        }
        ...
        $data['partner_trade_no'] = (isset($transfersInfo['partner_trade_no'])&&$transfersInfo['partner_trade_no'])?$transfersInfo['partner_trade_no']:get_sn(10);//$transfersInfo['id'];            //退款单号
        $data['nonce_str'] = self::getNonceStr();
        $data['enc_bank_no'] = $transfersInfo['card_number'];            //收款方银行卡号
        $data['enc_true_name'] = $transfersInfo['account_name'];            //收款方用户名
        $data['bank_code'] = self::$bank_code[$transfersInfo['bank_code']];            //收款方开户行
        $data['amount'] = $transfersInfo['money']*100;         //退款金额
        $data['desc'] = '用户提现';         //退款金额
        $data['sign'] = $this->makeSign($data);
        $xml = $this->toXml($data);
        ...
        $result['data'] = $data['partner_trade_no'];
        if($re['return_code'] == 'SUCCESS'){
            if($re['result_code'] == 'SUCCESS'){
                $result['status'] = true;
            }else{
     ...
     * 用户提现到微信钱包
     */
    public function transfers($transfersInfo)
    {
        $result   = [
            'status' => false,
            'data'   => [],
        ...
        //请求方式
        $http_method = 'POST';
        $no = (isset($transfersInfo['partner_trade_no']) && $transfersInfo['partner_trade_no']) ? $transfersInfo['partner_trade_no'] : get_sn(10);
        $money = intval(bcmul($transfersInfo['money'], 100, 0));
        $result['data'] = $no;
        //请求参数
        $data = [
            'appid' => getSetting('wx_appid'), //申请商户号的appid或商户号绑定的appid（企业号corpid即为此appid）
            'out_batch_no' => $no, //商户系统内部的商家批次单号，要求此参数只能由数字、大小写字母组成，在商户系统内部唯一
            'batch_name' => '提现至微信零钱', //该笔批量转账的名称
            'batch_remark' => '用户ID：' . $transfersInfo['user_id'] . '提现至微信零钱', //转账说明，UTF8编码，最多允许32个字符
            'total_amount' => $money, //转账金额单位为“分”。转账总金额必须与批次内所有明细转账金额之和保持一致，否则无法发起转账操作
            'total_num' => 1, //一个转账批次单最多发起三千笔转账。转账总笔数必须与批次内所有明细之和保持一致，否则无法发起转账操作
            'transfer_detail_list' => [
                [ //发起批量转账的明细列表，最多三千笔
                    'out_detail_no' => $no, //商户系统内部区分转账批次单下不同转账明细单的唯一标识，要求此参数只能由数字、大小写字母组成
                    'transfer_amount' => $money, //转账金额单位为分
                    'transfer_remark' => '提现至微信零钱', //单条转账备注（微信用户会收到该备注），UTF8编码，最多允许32个字符
                    'openid' => $transfersInfo['card_number'], //openid是微信用户在公众号appid下的唯一用户标识（appid不同，则获取到的openid就不同），可用于永久标记一个用户
                    //'user_name'=>2000//明细转账金额<0.3元时，不允许填写收款用户姓名，明细转账金额 >= 2,000元时，该笔明细必须填写收款用户真实姓名
                ]
            ]
        ];
        $cert_dir = ROOT_PATH . DS . "config" . DS . "payment_cert" . DS . "wechatpay" . DS;
