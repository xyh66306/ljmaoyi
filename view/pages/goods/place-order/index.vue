			...
			<view class="cell-group">
				...
				<view class="cell-item">
					<view class="cell-item-hd cell-item-bd-block">
						...
						<view class="cell-hd-view" v-if="cartData.point > 0 && orderType!=10">代金券抵扣</view>
						<view class="cell-hd-view" v-if="cartData.point > 0 && orderType==10">使用代金券</view>
						<view class="cell-hd-view" v-if="orderType==10">可用代金券</view>
		...
		<view class="button-bottom">
			<view class="button-bottom-c" v-if="orderType==10">
				...
				<view class="button-bottom-c-b">
					合计
					<text class="red-price">{{ cartData.amount || '' }}+{{ cartData.point || '' }}代金券</text>
				</view>
			</view>
			<view class="button-bottom-c" v-if="orderType!=10">
				<view class="button-bottom-c-t">共 {{ productNums || '' }} 件商品</view>
	...
	export default {
		...
		data() {
			return {
				...
				current: 0,
				isUsePoint: false, // 是否勾选使用代金券
				userPointNums: 0, // 用户的总代金券
				canUsePoint: 0, // 可以使用的代金券
				pointMoney: '', // 代金券抵扣的金额
					...
					if (res.status) {
						...
						if (this.isOpenPoint === 1 && !this.isUsePoint) {
							...
							this.$api.usablePoint(money, res => {
								if (res.status) {
									this.userPointNums = res.data; // 用户总代金券
									this.canUsePoint = res.available_point; // 可以使用的代金券
									this.pointMoney = res.point_rmb; // 代金券抵扣的总金额
								}
							});
						}
					...
					_this.$api.createOrder(data, res => {
							if (res.status) {
								...
								if (res.data.pay_status == '2') {
									...
									if(_this.exp==1){
										_this.$common.redirectTo('/pages/goods/payment/index?exp=1&order_id=' + res.data
											.order_id + '&type=' + _this.orderType);
									} else if(_this.exp==2) {
										_this.$common.redirectTo('/pages/goods/payment/index?exp=2&order_id=' + res.data
											.order_id + '&type=' + _this.orderType);
									} else {
		...
		watch: {
			// 监听数据状态(切换收货地址, 是否使用优惠券, 是否使用代金券) 重新请求订单数据
			params: {
				handler() {
					this.getCartList();
				},
