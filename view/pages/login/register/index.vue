...
<!-- 			<view class="reg-item flc">
			...
			<view class="reg-item">
				<input type="text" v-model="invitecode" placeholder="请输入邀请码" focus placeholder-class="login-item-i-p fsz26" />
			</view>			
			<view class="reg-item">
...
export default {
	...
	onLoad(options) {
		...
		if(this.$db.get('invitecode')){
			this.invitecode = this.$db.get('invitecode');
		}
