			...
			<view class="content-top">
				...
				<view class='cell-group margin-cell-group'>
					...
					<view class='cell-item refund-price'>
						...
						<view class='cell-item-ft'>
							<input type="digit" class='cell-bd-input red-price'
					...
					<view class='cell-item refund-price' v-if="orderInfo.order_type==10">
						<view class='cell-item-hd'>
							<view class='cell-hd-title w200'>可退代金券</view>
						</view>
						<view class='cell-item-ft color-9 fsz24 '>
							{{orderInfo.point}}代金券
						</view>
					</view>
					<view class="color-9 fsz24 refund-tip"  v-if="orderInfo.order_type==10">
						代金券会在整单退款完毕后退回账户
					</view>
				</view>
				<view class='cell-group margin-cell-group'>
				...
				if (res.status) {
					...
					if (res.data.text_status != 1 && res.data.text_status != 6 && res.data.text_status != 7){
						...
						for(var i=0;i<res.data.items.length;i++){
							...
							buyNums += res.data.items[i].nums
							
							reship_nums_all += res.data.items[i].reship_nums
						}		
						this.buyNums = buyNums
						this.reshipNumsAll = reship_nums_all
		...
		getReturnData(){
			...
			if(checkNums + this.reshipNumsAll == this.buyNums) {
				if(this.reSult.cost_freight_status == 1) {
					refund = total_refund.toFixed(2);
				} else {
					refund = (Number(total_refund)  + Number(this.cost_freight)) .toFixed(2);
				}
			} else {
				refund = total_refund.toFixed(2);
			}
			//判断是否超出最大可退金额
			if(Number(refund)>Number(this.maxRefund)){
				this.refund = this.maxRefund;
			}else{
				this.refund = refund;
			}
		},
