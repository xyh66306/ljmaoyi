<template>
	<view class="content">
		<view class="content-top">
			...
			<!-- 团购分享拼单 -->
			<view class="cell-group margin-cell-group" v-if="orderInfo.order_type==11 && orderInfo.status != 3 && orderInfo.pay_status != 1">
				...
				<view class='img-list-item' v-for="item in orderInfo.items" :key="item.id">
					...
					<view class='img-list-item-r little-right'>
						<view class='little-right-t'>
							...
							<view class='goods-name list-goods-name' @click="pintuanDetail(item.goods_id)" v-else-if="orderInfo.order_type == 11">{{ item.name || ''}}</view>
						...
						<view style="display: flex; justify-content: space-between;" class="goods-sales-info">
							<view v-if="orderInfo.order_type !== 10">
								<view class="price">
									<text class="symbol">￥</text>
									{{ item.price || '0.00' }}
								</view>
							</view>
							<view v-else="orderInfo.orderType === 10">
								<view class="price" v-if="item.price >0">
									<text class="symbol">￥</text>
									{{ item.price || '0.00' }}+{{ item.point || '0' }}代金券
								</view>
								<view class="price" v-if="item.price <=0">
									{{ item.point || '0' }}代金券
								</view>
							</view>
							<view class='goods-num'>x {{ item.nums || '' }}</view>
						</view>
					</view>
				</view>
			</view>
        ...
        <view class='cell-item add-title-item' v-if="orderInfo.point > 0">
          <view class='cell-item-bd'>
            <view class="cell-bd-view">
              <text class="cell-bd-text">使用代金券</text>
            </view>
          </view>
          <view class='cell-item-ft'>
            <text class="cell-ft-p">{{ orderInfo.point || ''}}</text>
          </view>
        </view>
        <view class='cell-item add-title-item' v-if="orderInfo.point_money > 0">
					<view class='cell-item-bd'>
						<view class="cell-bd-view">
							<text class="cell-bd-text">代金券优惠</text>
						</view>
					</view>
					<view class='cell-item-ft'>
				...
				<view class='cell-item add-title-item' v-if="orderInfo.pay_status > 1">
					...
					<view class='cell-item-ft' v-if="orderInfo.order_type != 10">
						<text class="cell-ft-p">{{ orderInfo.payment_name || ''}}</text>
					</view>
					<view class='cell-item-ft' v-if="orderInfo.order_type == 10">
						<text class="cell-ft-p" v-if="orderInfo.order_amount>0">代金券+{{ orderInfo.payment_name || ''}}</text>
						<text class="cell-ft-p" v-if="orderInfo.order_amount<=0">代金券支付</text>
					</view>
				</view>
				<view class='cell-item add-title-item' v-if="orderInfo.pay_status > 1">
					<view class='cell-item-bd'>
		...
		<view class="button-bottom" v-if="orderInfo.status == 1 || orderInfo.status == 2">
			...
			<button class='btn btn-circle btn-w' hover-class="btn-hover" v-if="orderInfo.status == 1 && orderInfo.pay_status == 1" @click="toPay(orderInfo.order_id,orderInfo.order_type)">立即支付</button>
			...
			<button class='btn btn-circle btn-w' hover-class="btn-hover" v-if="orderInfo.status === 1 && orderInfo.pay_status >= 2 && orderInfo.ship_status >= 3 && orderInfo.confirm >= 2 && orderInfo.is_comment === 1 && orderInfo.order_type!=10" @click="toEvaluate(orderInfo.order_id)">立即评价</button>
	...
	export default {
		...
		methods: {
			...
			logistics(key) {
				// #ifdef H5
				if(logisticsFree){
					window.location.href=logisticsFreeUrl+""+this.orderInfo.delivery[key].logi_no;
					return true;
				}else{
...
<style>
	...
	::v-deep.lvv-popup .lvv-popupmark{
