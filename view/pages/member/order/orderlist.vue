<template>
	<view class="content">
		<uni-segmented-control :current="tab" :values="items" @clickItem="onClickItem" style-type="text" active-color="#fda14a"></uni-segmented-control>
		<view class="order-list">
			<view class="goods-detail" v-if="list.length">
				<view class="order-item" v-for="(item, index) in list" :key="index">
					...
					<view class='cell-group'>
						<view class='cell-item'>
							<view class='cell-item-ft goods-num'>
								<text class='cell-ft-text' v-if="item.order_type!=10">合计<text class="red-price">￥ {{ item.order_amount }}</text></text>
								<text class='cell-ft-text' v-else>合计<text class="red-price">￥ {{ item.order_amount }}+ {{ item.point }} 代金券</text></text>
					...
					<view class='order-list-button'>
						...
						@click="buyAgain(item.items)" style="width: initial;" v-if="item.order_type==1"> 再次购买 </button>
						
						<button class='btn btn-circle btn-g' hover-class="btn-hover" @click="orderDetail(item.order_id)">查看详情</button>
						<button class='btn btn-circle btn-w' hover-class="btn-hover" v-if="item.status === 1 && item.pay_status === 1" @click="toPay(item.order_id,item.order_type)">立即支付</button>
			...
			<view class="order-none" v-else>
				<image class="order-none-img" src="/static/img/order-empty.png" mode=""></image>
			</view>
		</view>
	</view>
</template>
