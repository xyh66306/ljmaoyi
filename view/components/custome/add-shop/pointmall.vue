		...
		<view class="pop-c">
			...
			<view class="pop-t">
				...
				<view class="goods-information">
					...
					<view class="pop-goods-price red-price" v-if="product.open_point==1">
						<text class="fsz22"> {{ product.point || '' }}代金券</text>+
						<text class="fsz32">￥{{ product.price || '' }}</text>
					</view>
					<view class="pop-goods-price red-price" v-if="product.open_point==2">
						<text class="fsz22"> {{ product.point || '' }}代金券</text>
					</view>
				</view>
			</view>
			<scroll-view class="pop-m" scroll-y="true">
				<spec v-if="product && product.default_spes_desc" :spesData="product.default_spes_desc" ref="spec"
					@changeSpes="changeSpes"></spec>
				<view class="goods-number">
					<text class="pop-m-title">购买数量<text
							class="g5 fsz24 stock">剩余{{product.stock}} {{unit || ''}}</text> </text>

					<view class="pop-m-bd-in">
						<uni-number-box :min="minNums" :max="maxNums" :value="buyNum" @change="bindChange"
...
<script>
	...
	export default {
		...
		mounted() {
			//this.maxNums = this.product.stock > this.maxNums ? this.maxNums : this.product.stock;
		},
		watch: {
			maxNums: function(val) {
				return this.product.stock > val ? val : this.product.stock;
			}
		},
		methods: {
			show() {
			...
			bindChange(v) {
				if (v == this.product.stock) {
					this.$common.errorToShow(this.$t('buyMax'))
				}
